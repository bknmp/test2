...
public static class PhotonNetwork
{
	...
	internal static string CurrentRpcCaller;

	internal static string CurrentInstantiationCaller;

	internal static bool ErrorBadResources;
	...
	private static Coroutine coroutineDisconnect;
	...
	public static bool CrcCheckEnabled
	{
		...
		set
		{
			...
			{
				UnityEngine.Debug.Log("Can't change CrcCheckEnabled while being connected. CrcCheckEnabled stays " + networkingPeer.CrcEnabled.ToString());
			}
		}
	}
	...
	public static void Disconnect()
	{
		...
		else if (networkingPeer != null && coroutineDisconnect == null)
		{
			if (connected)
			{
				UnityEngine.Debug.Log("PhotonNetwork Disconnect");
			}
			if (inRoom && !Application.isEditor)
			{
				LeaveRoom();
				coroutineDisconnect = photonMono.StartCoroutine(DisconnectLater(0.5f));
			}
			else
			{
				networkingPeer.Disconnect();
	...
	private static IEnumerator DisconnectLater(float time)
	{
		yield return new WaitForSeconds(time);
		networkingPeer.Disconnect();
		PhotonVoiceNetwork.Disconnect();
		coroutineDisconnect = null;
	}
	...
	public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
	{
		...
		{
			value = ((networkingPeer.ObjectPool == null) ? ((GameObject)ResManager.Load(prefabName, typeof(GameObject))) : networkingPeer.ObjectPool.Load(prefabName));
			if (UsePrefabCache)
			{
	...
	public static void DestroyPlayerObjects(int targetPlayerId)
	{
		if (VerifyCanUseNetwork())
		{
			...
			{
				UnityEngine.Debug.LogError("DestroyPlayerObjects() failed, cause players can only destroy their own GameObjects. A Master Client can destroy anyone's. This is master: " + isMasterClient.ToString());
			}
		}
	}
	...
	public static void LoadLevel(int levelNumber)
	{
		...
		ResManager.LoadScene(levelNumber);
	}
	...
	public static void LoadLevel(string levelName)
	{
		...
		isMessageQueueRunning = false;
		networkingPeer.loadingLevelAndPausedNetwork = true;
		ResManager.LoadScene(levelName);
	}
